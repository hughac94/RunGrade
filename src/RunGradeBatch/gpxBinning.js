...
export function getBinSummary(bins) {
 ...
 */
export function getGradientPaceAnalysis(allResults) {
 ...
 */
export function getPaceByGradientChart(allResults) {
  // Collect all bins
  const allBins = [];
  allResults.forEach(result => {
    if (result.bins && Array.isArray(result.bins)) {
 ...
 */
export function getGradeAdjustmentAnalysis(allResults, basePaceOption = 'near-zero') {
  ...
  if (basePaceOption === 'near-zero') {
    ...
    if (nearZeroBins.length > 0) {
      ...
      const totalPace = nearZeroBins.reduce((sum, bin) => sum + bin.pace_min_per_km, 0);
      basePace = totalPace / nearZeroBins.length;

      // Calculate median
      const paces = nearZeroBins.map(bin => bin.pace_min_per_km).sort((a, b) => a - b);
      const mid = Math.floor(paces.length / 2);
      basePaceMedian = paces.length % 2 === 0
      ...
      basePaceBinStats = {
        ...
        totalTime,
        paceLabel: basePace ? `${Math.floor(basePace)}:${Math.round((basePace % 1) * 60).toString().padStart(2, '0')}` : 'N/A'
      };
    }
    ...
    basePace = (totalTime / 60) / (totalDistance / 1000); // min/km
    basePaceMedian = basePace; // For now, use mean as median
    basePaceMethod = 'All bins';
    basePaceBinStats = {
      label: 'All bins',
 ...
 */
export function getAdjustmentByGradientBins(allResults, basePace, statType = 'mean') {
  // Collect all bins
  const allBins = [];
  allResults.forEach(result => {
    if (result.bins && Array.isArray(result.bins)) {
